...
for r = 1:n_receivers
    ...
    plot_window(p_window(r,:),r,dt,'r',1);
    plot_window(s_window(r,:),r,dt,'b',1);
...
function plot_window(window,r,dt,color,line_width)
if isnan(window)
    return;
else
    plot([window(1)*dt window(1)*dt],[r*3-2.5 r*3+0.5],[color '--'],'LineWidth',line_width);
    plot([window(2)*dt window(2)*dt],[r*3-2.5 r*3+0.5],[color '--'],'LineWidth',line_width);
end
end